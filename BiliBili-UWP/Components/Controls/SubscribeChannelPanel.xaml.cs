    ...
    public sealed partial class SubscribeChannelPanel : UserControl
    {
        ...
        private void ChannelListView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if(item.@goto=="channel")
                App.AppViewModel.CurrentPagePanel.NavigateToSubPage(typeof(Pages.Sub.Channel.ChannelDetailPage), item.id);
            else if(item.@goto=="tag")
                App.AppViewModel.CurrentPagePanel.NavigateToSubPage(typeof(Pages.Sub.Channel.TagDetailPage), item.id);
        }
    }
}
